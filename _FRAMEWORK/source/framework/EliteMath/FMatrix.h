...
namespace Elite 
{
	...
	public:
		...
		/*void Substract(FMatrix& base, FMatrix& other)
		{
			int maxRows = min(base.GetNrOfRows(), other.GetNrOfRows());
			int maxColumns = min(base.GetNrOfColumns(), other.GetNrOfColumns());
			...
			for (int c_row = 0; c_row < maxRows; ++c_row)
			{
				...
				{
					float oVal = other.Get(c_row, c_column);
					float thisVal = base.Get(c_row, c_column);
					Set(c_row, c_column, thisVal - oVal);
				}
			}
		}*/
